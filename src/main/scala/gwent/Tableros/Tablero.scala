...
class Tablero {
  ...
  private var usuario = new Usuario("Usuario", this,
    _contadorGemas = 2, _mazoCartas = ListBuffer[Carta], _manoCartas = ListBuffer[Carta])
  private var cpu = new CPU("CPU", this,
    _contadorGemas = 2, _mazoCartas = ListBuffer[Carta], _manoCartas = ListBuffer[Carta])
   ...
   * @param carta
   */
  def agregarEnDistanciaUsuario(carta: CartaUnidadDistancia): Unit = {
